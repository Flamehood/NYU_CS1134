...
def min_and_max(bin_tree):
    ...
    if bin_tree.root.left is None and bin_tree.root.right is None:
        return (root, root)
    root = bin_tree.root
    return subtree_min_and_max(root)
...
def subtree_min_and_max(node):
    if node.left is None and node.right is None:
        ...
        curr_min_max = subtree_min_and_max(node.right)
        minimum = min(subtree_min_and_max(node.right)[0], node.data)
        maximum = max(subtree_min_and_max(node.right)[1], node.data)
    elif node.right is None:
        curr_min_max = subtree_min_and_max(node.left)
        minimum = min(subtree_min_and_max(node.left)[0], node.data)
        maximum = max(subtree_min_and_max(node.left)[1], node.data)
    else:
        min_max_left = subtree_min_and_max(node.left)
        min_max_right = subtree_min_and_max(node.right)
        minimum = min(min_max_left[0], min_max_right[0])
        maximum = max(min_max_left[1], min_max_right[1])
